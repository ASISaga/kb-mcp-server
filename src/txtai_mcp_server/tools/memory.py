...
from ..core.state import get_txtai_app, add_to_document_cache, get_document_cache
...
def register_memory_tools(mcp: FastMCP) -> None:
    ...
    async def store_memory(
        ...
        timestamp: Optional[str] = Field(None, description="ISO timestamp when memory was created/occurred. Defaults to now."),
        ...
        sentiment: Optional[str] = Field(None, description="Emotional context: 'positive', 'negative', 'neutral', etc."),
        source: Optional[str] = Field(None, description="Source of this memory (e.g., 'conversation', 'reading', 'experience')"),
    ...
    async def recall_by_time(
        ctx: Context,
        period: str = Field(..., description="Time period: 'today', 'yesterday', 'last_week', 'last_month', 'last_year', or ISO date range 'YYYY-MM-DD:YYYY-MM-DD'"),
        ...
        try:
            ...
            sql_parts = [f"timestamp >= '{start_date.isoformat()}'", f"timestamp <= '{end_date.isoformat()}'"]
            ...
            results = app.embeddings.search(
                "SELECT id, text, score FROM txtai WHERE " + sql_filter + f" ORDER BY timestamp DESC LIMIT {limit}"
            )
            ...
            for result in results:
                ...
                if hasattr(app.embeddings, 'search'):
                    try:
                        meta_results = app.embeddings.search(f"SELECT metadata FROM txtai WHERE id = '{memory_id}'")
                        if meta_results:
    ...
    async def reflect_on_memories(
        ctx: Context,
        aspect: str = Field(..., description="What to reflect on: 'topics', 'importance', 'sentiment', 'frequency', 'timeline', or 'all'"),
        time_period: Optional[str] = Field(None, description="Time period to analyze (same format as recall_by_time)"),
        ...
        try:
            ...
            if time_period:
                ...
                # Get all memories
                results = app.embeddings.search(f"SELECT id, text, metadata FROM txtai WHERE type = 'memory' LIMIT {limit}")
                memories = []
                for result in results:
                    metadata = {}
                    try:
            ...
            if aspect in ["importance", "all"]:
                ...
                importance_scores = [m.get("importance", 0) for m in memories if m.get("importance")]
            ...
            if insights.get("top_topics"):
                ...
                recommendations.append(f"Your most frequent topic is '{top_topic}' - consider consolidating related memories")
            
            if insights.get("importance_stats", {}).get("high_importance_count", 0) > 5:
                recommendations.append("You have many high-importance memories - consider reviewing and organizing them")
            
            if insights.get("access_patterns", {}).get("rarely_accessed_count", 0) > 10:
                recommendations.append("Many memories are rarely accessed - consider reviewing their relevance")
    ...
    async def update_memory_importance(
        ...
        try:
            ...
            # Get existing memory
            results = app.embeddings.search(f"SELECT text, metadata FROM txtai WHERE id = '{memory_id}'")
            if not results:
                return {
                    "status": "error",
            ...
            return {
                "status": "error",
                "error": str(e),
                "message": "Failed to update importance"
            }
