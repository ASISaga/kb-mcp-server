...
from ..core.state import get_txtai_app, add_to_document_cache
...
def register_conversation_tools(mcp: FastMCP) -> None:
    ...
    async def store_conversation_turn(
        ...
        user_sentiment: Optional[str] = Field(None, description="Detected sentiment of user message"),
    ...
    async def recall_conversation_history(
        ...
        since: Optional[str] = Field(None, description="ISO timestamp - only return conversations after this time"),
        ...
        try:
            ...
            return {
                "status": "error",
                "error": str(e),
                "message": "Failed to recall conversations"
            }
    ...
    async def summarize_conversation_session(
        ...
        try:
            ...
            history_result = await recall_conversation_history(ctx, session_id=session_id, limit=100)
    ...
    async def search_conversations(
        ...
        try:
            ...
            for result in results:
                ...
                # Get metadata
                metadata_results = app.embeddings.search(f"SELECT metadata FROM txtai WHERE id = '{result_id}'")
                metadata = {}
                if metadata_results:
                    try:
